// BrainCanvas.jsx — real fsaverage5.glb with TRIBE v2-style activation rendering.
// Hot colormap: white cortex → white-hot → yellow → orange → red → deep ember.
// Dramatic lighting reveals gyral folds. Parcel state-machine drives weather-like waves.

const FSAVG_URL = 'assets/fsaverage5.glb';

// LUT matches the TRIBE v2 reference render: inactive cortex is bright warm white,
// activation goes white-hot → yellow → orange → red → deep crimson at peak.
const HOT_STOPS = [
  [1.00, 0.98, 0.90],   // white-warm (just above threshold)
  [1.00, 0.96, 0.55],   // bright yellow-white
  [1.00, 0.82, 0.18],   // yellow
  [1.00, 0.55, 0.06],   // orange
  [0.88, 0.18, 0.04],   // red
  [0.52, 0.04, 0.02],   // deep crimson
];
const HOT_LUT = (() => {
...
// Bright warm white — matches the inactive cortex in TRIBE v2 renders.
...
// Anatomical parcels — fewer, smaller, longer cooldowns so we always see
// distinct active patches separated by white inactive cortex.
    ...
    lobes.forEach(L => {
      for (let i = 0; i < L.count; i++) {
        ...
        parcels.push({
          ...
          // Tighter radius so patches are distinct, not a uniform red blanket
          ...
          // Long cooldown = lots of white cortex visible between activations
          cooldown: 7    + Math.random()*9,
          // Stagger starting timers so not everything fires at once
  ...
  for (const p of parcels) {
    ...
    if (p.state === 'quiet') {
      ...
      if (p.phase >= 1) {
        ...
        // Only excite at most 1 neighbour, with low probability, to avoid cascade
...
function writeColors(dirs, colors, parcels, count, brightness) {
  for (let i=0;i<count;i++){
    ...
    // Higher threshold → only the peak of each parcel gets coloured,
    // leaving most of the white cortex clean between patches.
    if (act < 0.14) {
      ...
      // Gentle power curve so mid-activation lands in the yellow/orange band
      const tt = Math.pow(t, 1.1);
      const idx = Math.min(255, (tt*255)|0);
      // Full saturation mix — no blending with base at the active edge
...
function BrainCanvas({ width=360, height=360, autoRotate=true, interactive=false, brightness=1, initRotX=-93, initRotY=4 }) {
  const mountRef = React.useRef(null);
  const groupRef = React.useRef(null);
  // Tracks the accumulated world-Y spin so we can reapply it after an orientation reset
  const spinRef = React.useRef(0);

  // Live orientation update — fires on every slider change without recreating the scene
  React.useEffect(() => {
    const g = groupRef.current;
    if (!g) return;
    // Reapply base orientation; the render loop will keep spinning on top of this
  ...
  React.useEffect(() => {
    ...
    // Physically correct lighting so roughness/metalness behave as expected
    renderer.physicallyCorrectLights = true;
    mount.appendChild(renderer.domElement);

    // Lighting tuned to reveal gyral folds — matches TRIBE v2 reference render style.
    // Low ambient keeps sulci dark; strong key from upper-left-front highlights gyral crowns.
    scene.add(new THREE.AmbientLight(0xffeedd, 0.38));

    const key = new THREE.DirectionalLight(0xffffff, 2.2);
    key.position.set(3, 5, 5); scene.add(key);

    // Warm fill from lower-right to add depth
    const fill = new THREE.DirectionalLight(0xffd8b0, 0.55);
    fill.position.set(-3, -1, 2); scene.add(fill);

    // Cool rim from behind — creates the subtle glow seen in the reference
    const rim = new THREE.DirectionalLight(0xc8e8ff, 0.30);
    rim.position.set(-2, 4, -5); scene.add(rim);

    // Top light to brighten superior surface
    const top = new THREE.DirectionalLight(0xffffff, 0.60);
    ...
    let geo=null, mat=null, mesh=null, parcels=null, dirs=null, count=0;
    ...
    (async () => {
      try {
        ...
        mat = new THREE.MeshStandardMaterial({
          vertexColors: true,
          roughness: 0.50,   // lower = more specular highlights on gyral crowns
          metalness: 0.04,
          flatShading: false,
        });
    ...
    const tick = () => {
      ...
      // Spin around world Y (vertical) so it rotates in the same plane
      // as the perspective-tilted site behind it — not a diagonal tumble.
  ...
  }, [width, height, autoRotate, interactive, brightness]);
